    ...
    mapInstance.on('load', () => {
      ...
      LISTINGS.forEach((p) => {
        const linkHtml = p.slug
          ? `<a style="display:inline-block;padding:6px 14px;border-radius:6px;background:#243C4C;color:#fff;font-size:12px;font-weight:600;text-decoration:none;transition:background 0.2s" href="/properties/${p.slug}">View Details &rarr;</a>`
          : `<a style="display:inline-block;padding:6px 14px;border-radius:6px;background:#243C4C;color:#fff;font-size:12px;font-weight:600;text-decoration:none;transition:background 0.2s" href="/buy">View Listings &rarr;</a>`;
        ...
        const marker = new maplibregl.Marker({ color: '#243C4C' })
